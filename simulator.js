...
var globalTimeMax=300; // time interval e.g. 15 min
var numRealizations=1000; // number of model instances - e.g. number of agents
...
function calculateFull(indata){
 ...
 //alert('Agents: mNew='+m+', oldM='+Math.ceil(lambda*aht*(1-maxAR/100))+', gamma='+newGamma); 
    ...
    var mnc=0,mna=0;
    var asl=0;
    var aar=0,aasa=0,aao=0;
    for(var i=0;i<hours;i++){  
  		//callsN=hoursGrid.store.getAt(i).data.calls;
  		if(hoursGrid.store.getAt(i).data.calls){
  		  var rec=hoursGrid.store.getAt(i);
  		  if(rec.get('calls')>mnc) mnc=rec.get('calls');
  		  if(rec.get('agents')>mna) mna=rec.get('agents');
  		  asl+=parseFloat(rec.get('sl'));
  		  aar+=parseFloat(rec.get('ar'));
  		  aasa+=parseFloat(rec.get('asa'));
  		  aao+=parseFloat(rec.get('occupancy'));
  		  j++;	
       };
	};  
	if(j>0){
    ...
    //alert('End of Cycle!,j='+j+', sl='+asl);
    refreshSummary(mnc,mna,asl,aar,aasa,aao); 
...
function calculateOC(indata){
	var aht=indata.aht;
	var clsize=indata.clsize;
	var cltime=indata.cltime;
	var hitratio=indata.hitratio;
	var aat=indata.aat;
	var maxar=indata.maxar;
	var maxawt=indata.maxawt;
	var countawt=indata.countawt;
	var lambda=clsize*hitratio/(cltime*60*100);
	var m=Math.ceil(aht*lambda+gamma*Math.sqrt(aht*lambda));
	//Ext.Msg.alert('Start Calculating Outbound Staffing Level ','lambda='+lambda+', m='+m);
	var br=0;
    var result={}, oldResult={};
    steps=1;b1=0;b2=0;b3=0;b4=0;b5=0;b6=0;
    while(true){ 
    	var result=simulate(m,lambda,aht,aat,20);  
   	    if        ((result.ar<=maxar) && (result.asa<=maxawt||!countawt)  && br==1){b1++;m--;}
     	else if   ((result.ar<=maxar) && (result.asa<=maxawt||!countawt)  && br==2){b2++;oldResult=result;break;}
    	else if   ((result.ar<=maxar) && (result.asa<=maxawt||!countawt)  && br==0){b3++;m--;br=1;} 
    	else if (!((result.ar<=maxar) && (result.asa<=maxawt||!countawt)) && br==0){b4++;m++;br=2;} 
    	else if (!((result.ar<=maxar) && (result.asa<=maxawt||!countawt)) && br==1){b5++;m++;break;}    
    	else if (!((result.ar<=maxar) && (result.asa<=maxawt||!countawt)) && br==2){b6++;m++;}
    	else {alert('Error in cycle')};
    	oldResult=result;
        steps++;
    };
    oldResult.m=m;
    oldResult.calls=clsize;	
	return oldResult;
};
//return {'asa':aver_asa, 'ar':aver_ar,'ao':aver_occ,'sl':aver_sl,'awc':aver_awc,'aql':aver_aql,'ata':aver_ata,'points':points};	
 //       	var parm={'aht':averageHandlingTime,'clsize':clSize,'cltime':clTime,'hitratio':hitRatio,'aat':averageAbTime,'maxar':maxAR,'maxawt':maxAWT,'countawt':countAWT};
function simulateOC(indata){
// (m,)
	var result=simulate(pm,plambda,paht,paat,20);
	return result;
};
